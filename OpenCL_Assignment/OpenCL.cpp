...
/*********************************************************************
 * @Brief Returns true if String starts with Test
 *********************************************************************/
...
{
	rwFile.open("read_write_Results.txt", std::ios::app);
...
bool OpenCL_ReadData(cl_mem Buffer, size_t Size, void * Data)
{
	...
	cl_int blockError = clFinish(Commands);
	rwFile << "OpenCL ReadData time: " << CPU_toc() << " ms\n";
...
bool OpenCL_WriteData(cl_mem Buffer, size_t Size, void * Data)
{
	...
	cl_int blockError = clFinish(Commands);
	rwFile << "OpenCL WriteData time: " << CPU_toc() << " ms\n";

	if (blockError != CL_SUCCESS) {
		printf("Error: WriteData did not finish successfully.\n");
		return false;
	}
